...
T getSafe(const json& j, const std::string& key, const T& default_value) {
    if (!j.contains(key)) {
        if constexpr (std::is_enum_v<T>) {
            PLOG_WARNING << "doc[\"" << key << "\"] is not available & default value set: \""
    ...
    if (jvalue.is_null()) {
        if constexpr (std::is_enum_v<T>) {
            PLOG_WARNING << "doc[\"" << key << "\"] is null & default value set: \""
                         << static_cast<std::underlying_type_t<T>>(default_value) << "\"";
            return default_value;
        } else {
            PLOG_WARNING << "doc[\"" << key << "\"] is null & default value set: \""
    ...
    if constexpr (std::is_enum_v<T>) {
        if (jvalue.is_string()) {
            try {
                ...
                PLOG_WARNING << "doc[\"" << key.c_str()
                             << "\"] enum conversion failed: " << e.what() << " | value: \""
                             << jvalue.dump() << "\"";
            }
            return default_value;
        }
    ...
    PLOG_WARNING << "doc[\"" << key << "\"] has invalid type: \"" << jvalue.type_name()
                 << "\" & value: \"" << jvalue.dump() << "\"";
    return default_value;
}
}  // namespace JsonConfig
